...
# App Title
# -------------------------------
st.markdown("<h1 style='text-align:center;color:#38bdf8;'>📝 Focus Pad</h1>", unsafe_allow_html=True)
st.markdown("<p style='text-align:center;color:#9ca3af;'>Simple • Clean • Productive</p>", unsafe_allow_html=True)
...
new_task = st.text_input("Task name", placeholder="What do you want to do today?")

if st.button("Add Task 🚀"):
    if new_task.strip():
        db = SessionLocal()
        db.add(Todo(title=new_task.strip()))
        db.commit()
        db.close()
    ...
    with st.container():
        ...
        with col2:
            if st.session_state.get(f"edit_{task.id}", False):
                ...
                st.markdown(f"<span class='{title_class}'>{task.title}</span>", unsafe_allow_html=True)
...
st.markdown("<p style='text-align:center;color:#6b7280;'>Built with ❤️ using Streamlit</p>", unsafe_allow_html=True)
